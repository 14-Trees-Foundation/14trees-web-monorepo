...
export default function DonatePage() {
  ...
  const [assignee, setAssignee] = useState({name: "", email: "", phone: "", tree_count: 0, relation: "other"});
  ...
  useEffect(() => {
    if (isAssigneeDifferent && formData.numberOfTrees === "1") {
      setAssignee(prev => ({ ...prev, tree_count: 1 }));
    }
  }, [isAssigneeDifferent, formData.numberOfTrees]);
  ...
  const validateDedicatedNames = () => {
    ...
    if (isAssigneeDifferent) {
      ...
      if (assignee.tree_count > Number(formData.numberOfTrees)) {
        newErrors.assigneeTrees = "Cannot exceed total tree count";
        isValid = false;
      }
    }
  ...
  console.groupEnd();
    ...
    try {
      ...
      const donationRequest = {
        ...
        users: isAssigneeDifferent
  ? [
      // Recipient (payer) - gets 0 trees
      {
        recipient_name: dedicatedNames[0].name, // From name field
          recipient_email: dedicatedNames[0].email,
          recipient_phone: dedicatedNames[0].phone,
          trees_count: 0,
          relation: "sponsor"
      },
      // Assignee - gets ALL trees
      {
        assignee_name: assignee.name, // Fixed typo
        assignee_email: assignee.email || "",
        assignee_phone: assignee.phone || "",
        image_url: "",
        // Removed redundant assignee field
        trees_count: Number(formData.numberOfTrees), // Fixed: All trees to assignee
        relation: assignee.relation
      }
    ]
          : recipientsWithImages.map(recipient => ({
                    ...
                    <div className="border border-gray-200 rounded-md p-4 space-y-4">
                      ...
                      <div className="grid gap-4 md:grid-cols-2">
                        <input
                          ...
                          onChange={(e) => setAssignee({...assignee, email: e.target.value})}
                          className="w-full rounded-md border border-gray-300 px-4 py-3"
                        />
                        <input
                          type="tel"
                          ...
                          onChange={(e) => setAssignee({...assignee, phone: e.target.value})}
                          className="w-full rounded-md border border-gray-300 px-4 py-3"
                        />
                      </div>
                      {Number(formData.numberOfTrees) > 1 && (
                        <div>
                          <label className="block text-sm font-medium mb-1">
                            Trees for Assignee (max: {formData.numberOfTrees})
                          </label>
                          <input
                            type="number"
                            min="1"
                            max={Number(formData.numberOfTrees)}
                            value={assignee.tree_count}
                            onChange={(e) => setAssignee({...assignee, tree_count: Number(e.target.value)})}
                            className="w-full rounded-md border border-gray-300 px-4 py-3"
                          />
                        </div>
                      )}
                      <div>
                        ...
                        <select
                          value={assignee.relation}
                          onChange={(e) => setAssignee({...assignee, relation: e.target.value})}
                          className="w-full rounded-md border border-gray-300 px-4 py-3"
                        >
