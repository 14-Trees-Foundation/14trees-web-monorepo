...
import { Pond, PondUpdate } from "../models/pond";
...
export const getPonds = async (req: any,res: any) => {
  ...
  try {
    // let result = await Pond.find(filters, { updates: 0 }).skip(offset).limit(limit);
...
export const deletePond = async (req: any,res: any) => {
  try {
      ...
      let resp = await PondRepository.deletePond(req.params.id , res) ;
...
export const getHistory = async (req: any,res: any) => {
  ...
  try {
    // let result = await Pond.find({ name: req.query.pond_name });
    let result = await PondRepository.getHistory({ name: req.query.pond_name  } ,  res);
...
//     const ponds = await Pond.find({name: { $regex: regex }}).skip(offset).limit(limit);
